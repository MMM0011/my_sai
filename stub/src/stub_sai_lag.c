    ...
    _In_ sai_attribute_t *attr_list)
{
    ...
    if (ii == MAX_NUMBER_OF_LAG_MEMBERS) {
        printf("Cannot create LAG_MEMBERS: limit is reached\n");
        return SAI_STATUS_FAILURE;
    }
    ...
    status = stub_create_object(SAI_OBJECT_TYPE_LAG_MEMBER, lag_db_id, lag_member_id);
    if (status != SAI_STATUS_SUCCESS) {
    ...
    _In_ sai_object_id_t lag_member_id)
{
    sai_status_t status;
    uint32_t     lag_db_id;
    status = stub_object_to_type(lag_member_id, SAI_OBJECT_TYPE_LAG_MEMBER, &lag_db_id);
    if (status != SAI_STATUS_SUCCESS) {
        printf("Cannot get LAG DB ID.\n");
        return status;
    }
